...
function addTalk(){
	...
	if(talkScript == null){
		...
		for(key in charaLayer.childList){
			console.log("npc x is "+charaLayer.childList[key].x+", tx is "+tx);
			console.log("npc y is "+charaLayer.childList[key].y+", ty is "+ty);
			//判断前有没有npc，有则开始对话
			if(key!=0){//key==0时表示是player
			if(charaLayer.childList[key].x == tx && charaLayer.childList[key].y == ty){
				console.log("yes,you are close to me");
				if(talkScriptList["talk"+charaLayer.childList[key].index]){
					console.log(charaLayer.childList[key].index);
					//alert("talk"+charaLayer.childList[key].index]);
