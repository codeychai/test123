 ...
 **/
init(50,"mylegend",480,288,main);//游戏画面尺寸为480*288
/**层变量*/
//显示进度条所用层
var loadingLayer;
//游戏底层
...
function initMap(){	
	...
	if(imageArray == null){
		...
		imageArray = LGlobal.divideCoordinate(bitmapdata.image.width,bitmapdata.image.height,20,20);
	}
}
//添加地图
function addMap(cx,cy){
	...
	//在地图层上，画出15*10的小图片-->30*35
	for(i=my;i<i<9 +Math.abs(cy) && i-mapY < map.length;i++){
		for(j=mx;j<15 +Math.abs(cx)&& j-mapX < map[0].length;j++){
			//从地图数组中得到相应位置的图片坐标
			index = map[i-mapY][j-mapX];
			//小图片的竖坐标
			indexY = Math.floor(index /20);
			//小图片的横坐标
			indexX = index - indexY*20;
			//得到小图片
